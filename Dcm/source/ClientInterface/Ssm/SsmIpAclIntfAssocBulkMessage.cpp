...
namespace DcmNs
{
    ...
    SsmIpAclIntfAssocBulkMessage::SsmIpAclIntfAssocBulkMessage ()
        : DcmManagementInterfaceMessage (SSMIPACLINTFASSOCBULK),
		m_numRecords (0)
    {
    }
    ...
    void  SsmIpAclIntfAssocBulkMessage::setupAttributesForSerialization()
    {
    ...
    vector<string>  SsmIpAclIntfAssocBulkMessage::getPolicyNameVector() const
    {
        return (m_policyName);
    }

    vector<UI32>  SsmIpAclIntfAssocBulkMessage::getOpCodeVector() const
    {
        return (m_opcode);
    }


    vector<string>  SsmIpAclIntfAssocBulkMessage::getIfNameVector() const
    {
        return (m_ifname);
    }

    void  SsmIpAclIntfAssocBulkMessage::setNumRecords(const UI64 &num)
    {
        m_numRecords  =  num;
    }


    UI64  SsmIpAclIntfAssocBulkMessage::getNumRecords() const
    {
        return (m_numRecords);
    }

	void SsmIpAclIntfAssocBulkMessage::setIntfDetails(const string &ifname, const string &policy, const UI32 &opcode, const UI32 &direction)
	{
	...
	const void *SsmIpAclIntfAssocBulkMessage::getCStructureForInputs ()
	{
        ...
        if (num > 0) {
        	...
        	for (i = 0; i < num; i++) {
				...
				/* Policy Type should alwasy be L3IPv4 in this Class*/
				pInput->binding_list[i].type    	= SSMACL_TYPE_L3_IPV4_E; 
			}
	...
	void SsmIpAclIntfAssocBulkMessage::loadOutputsFromCStructure (const void *pOutputCStructure)
	{
	}
}
