...
#include "discover/vsphere/vim25/mo/EventHistoryCollector.h"

namespace DcmNs {

    class EventManager :public ManagedObject{
        public:
            EventManager(vimStub *_stub,ManagedObjectReference *mor);
            virtual ~EventManager();
            int createCollectorForEvents(
                        EventFilterSpec *filter, ManagedObjectReference **r_mor);
            int createCollectorForEvents(
                                            EventFilterSpec *filter, EventHistoryCollector **r_mor);
    };
