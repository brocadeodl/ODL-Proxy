...
#ifndef MACADDRESSTABLESTATICCONFIGHANDLER_H
#define MACADDRESSTABLESTATICCONFIGHANDLER_H
...
class MacAddressTableStaticConfigHandler : public WaveConfigurationSegmentWorker
{
    ...
    private :
        AttributeModel                  getAttributeModel (WaveConfigurationSegmentContext *pWaveConfigurationSegmentContext);

        virtual void                    trace (TraceLevel traceLevel, const string &stringToTrace);

    protected :
    public :
                        MacAddressTableStaticConfigHandler  (WaveObjectManager *pWaveObjectManager);
        virtual        ~MacAddressTableStaticConfigHandler  ();
